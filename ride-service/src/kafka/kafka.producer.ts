...
export class KafkaProducer implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.kafka = new Kafka({
      ...
      clientId: 'ride-service-producer',
    });
  ...
  async onModuleInit() {
    try {
      ...
      logger.error('Failed to connect Kafka producer', { error: (error as Error).message });
      throw error;
    }
  }
  ...
  async publish(topic: string, message: object): Promise<void> {
    try {
      ...
      logger.error('Failed to publish Kafka message', { error: (error as Error).message, topic });
      throw error;
    }
  }
}